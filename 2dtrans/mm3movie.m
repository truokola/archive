function mov = mm3movie(n,en,tr2,tr3)

% mm3movie(n)
%
% Calculates transmission through a 2D three-terminal device
% using n modes. Return movie frames.
...
%energy = 1.00001*h2m*pi^2/(w1^2)*mode^2 + (0:.004:20);
%energy = .82;
energy = en;
en = en - 1.00001*h2m*pi^2/(w1^2)*mode^2;
...
Hlo = subplot('position', [0 0 1 .7]);
Hhi = subplot('position', [.03 .735 .94 .25]);
...
nn = 1;
for E = energy
  E1 = E - 1.00001*h2m*pi^2/(w1^2)*mode^2;
...
%  wave1 = abs(wave1).^2;wave2 = abs(wave2).^2;
  wave1(dim1:dim1+dimL-1,len1+1:len1+dim2) = wave2;
  ...
  subplot(Hlo);
  [Dx,Dy] = gradient(wave1, delta, delta);
  jx = imag(conj(wave1).*Dx);
  jy = imag(conj(wave1).*Dy);
%  [a,b] = view;
  % Current density plot
  dens = sqrt(jy.*jy+jx.*jx);
%  surf(linspace(-l1,w2+l1,2*len1+dim2), linspace(0,w1+L,dim1+dimL-1),dens);
  surf(linspace(-l1,w2+l1,2*len1+dim2), linspace(0,w1,dim1),dens(1:dim1,:));
  hold on;
  surf(linspace(0,w2,dim2), linspace(w1,w1+L,dimL),...
       dens(dim1:dim1+dimL-1, len1+1:len1+dim2));
  hold off;
  view(0, 90);
  shading interp;
  axis off;
  %  axis([-l1 w2+l1 0 w1+L 0 .2]);
